...
def afficher_en_tete(ville, temperature, condition_meteo):
    ...
    message_ligne = (" " + message1 + " - " + message2 + " - " + message3)
...
def get_jour(period):
    ...
    ex : si l'on veur savoir quel est le jour de la semaine à J+2 en fonction du jour actuel (si on est mercredi, cela
    renvoi vendredi)
...
def afficher_image_meteo(statut_image_meteo):
    ...
    :param statut_image_meteo: statut météo qui permet de choisir l'image appropriée,
    c'est à dire le bon fichier à ouvrir
    """
    chemin_reprtoire = os.path.dirname(__file__) + "/ressources/textes/"
    nom_image = meteo_utils.get_texte_meteo_file_name(statut_image_meteo)
...
def afficher_ecran_accueil():
    """
    Affiche le premier écran qui permet à l'utilisateur de faire son choix entre recherche de ville et
